...
(function () {
  ...
  function keyTierGroup(key) {
    // Lightweight structural check (does NOT verify the check code).
    const parts = (key || "").trim().toUpperCase().split("-");
    if (parts.length !== 5 || parts[0] !== "AETH") return null;
    if (!/^(PRO|ULTI)$/.test(parts[3])) return null;
    for (const p of [parts[1], parts[2], parts[4]]) {
      if (!/^[A-Z2-9]{4}$/.test(p)) return null;
    }
    return parts[3];
  }
  ...
  window.AetherLicense = { keyValid, tierOf, isDevKey, checksumGroup,
                           generateKey, keyTierGroup, DEV_KEY };
})();
